...
这个文件是 Agent 的“装配中心”，负责把模型、工具、子 Agent、
...
from agent.src.core.graph import get_langgraph_store
from agent.src.core.model import make_main_model
...
from agent.src.middleware import (
...
# 只保留本地 workspace，不接入 之前项目 的远程 sandbox。
...
def graph_loaded_for_execution(config: RunnableConfig) -> bool:
    ...
    FOREMAN 不使用 langgraph dev，但保留这个判断，可以让代码结构
...
def _code_reviewer_subagent(model: BaseChatModel) -> SubAgent:
    ...
    return {
        ...
        "system_prompt": (
            "你是 FOREMAN 的代码审查子 Agent，只负责 review，不负责修改代码。\n"
            "你必须使用中文输出，代码标识符、路径、命令和 API 名称可以保留英文。\n"
            "审查流程：\n"
            "1. 按 code-review skill 先用 read_file 读取工作区规则和仓库规则；"
            "读不到时调用 load_default_review_rules。\n"
            "2. 如果用户提供 Gitee PR 编号，调用 get_gitee_pull_request_context "
            "读取 PR 详情、提交、文件和评论。\n"
            ...
            "6. 记录 finding 前，尽量确认文件属于本次 diff；"
            "无法确认行号时使用文件级 finding。\n"
            "7. 使用 add_review_finding 保存结构化发现，再用 "
            "list_review_findings 汇总。\n"
...
def get_agent(config: RunnableConfig):
    ...
    远程 sandbox、团队模型配置等。全部使用本地配置，因此这里保留同名
    ...
    if (
        ...
        or not thread_id
        or not graph_loaded_for_execution(config)
    ):
        # 讲课重点：
    ...
    agent_backend, memory_paths, repo_memory_content = _prepare_repo_backend_context(
        repo_url=repo_url, backend=backend
    )
    ...
    # 暂时主 Agent 和子 Agent 共用 deepseek-v4-pro。
    ...
    from agent.src.core.graph import get_checkpointer

    logger.info(
        "返回带 backend 的 Agent：thread_id=%s task_kind=%s", thread_id, task_kind
    )
    ...
    return create_deep_agent(  # 创建一个Agent
        ...
        # 你有一个名为 compact_conversation 的工具可用。该工具会刷新你的上下文窗口，
        # 以减少上下文膨胀和成本。
        #
        # 在以下情况中应使用该工具：
        #
