...
2. 用确定性的 Python 代码解析 diff，提取“本次变更涉及哪些文件”和“新文件中哪些行发生了变
更”。
3. 在 reviewer 子 Agent 记录 finding 前，校验 finding 的文件和行号是否真实属于本次
diff。
...
- 把 diff 解析写成确定性工具，可以让 reviewer 子 Agent 专注于判断风险，
而不是处理行号细节。
"""
...
def parse_unified_diff(
    diff_text: str, *, base: str = "HEAD", head: str | None = None
) -> DiffSummary:
...
def get_local_diff(
    backend: LocalShellBackend, repo_dir: str, base: str = "HEAD"
) -> str:
...
def get_local_diff_summary(
    ...
    *,
    ...
    Args:
        ...
        head: 可选目标分支。传入时使用 `base...head`，不传时审查工作区相对 base 的
        diff。
    """
    ...
    # cwd 使用 backend 的虚拟路径，例如 projects/ai_coding，由 backend 做 Windows
    # 路径映射和权限校验。
...
def validate_finding_location(
    summary: DiffSummary, *, file: str, line: int | None
) -> tuple[bool, str]:
