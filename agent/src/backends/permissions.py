...
讲课时要强调：
这里不是完整的企业沙箱，只是本地 Windows backend 的安全收敛层。
...
def ensure_safe_git_message(message: str) -> str:
    ...
    Git 本身支持复杂 message，但当前通过 Windows shell 执行 git commit，
    ...
    if not normalized:
        return "FOREMAN generated changes"
