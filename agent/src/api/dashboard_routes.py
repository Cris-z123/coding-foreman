...
2. POST SSE 接口：
   ...
   - `/threads/{thread_id}/stream-message` 在已有会话中追加一轮用户输入，
     并实时运行 Agent。
...
def _normalize_dashboard_repo_url(
    repo: str | None, *, fallback: str | None = None
) -> str:
...
class DashboardThreadMessageRequest(BaseModel):
    ...
    其它字段保留给页面模型选择、图片输入和推理强度扩展，当前只使用部分字段。
    """
...
def _pr_payload(thread: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        "number": number,
        "title": thread.get("title") or "FOREMAN Pull Request",
        "state": "open",
...
def _message_payload(thread: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, message in enumerate(visible_checkpoint_messages(thread_id)):
        ...
        author = (
            message.get("author")
            if message.get("author") in {"user", "agent", "system", "tool"}
            else "agent"
        )
        messages.append(
            {
                "id": message.get("message_id")
                or f"{thread_id}-history-fallback-{index}",
                "author": author,
...
def _thread_payload(thread: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "id": thread["thread_id"],
        "title": thread.get("title") or "FOREMAN Task",
        "repo": repo_full_name,
...
def dashboard_me() -> dict[str, Any]:
    ...
    没有真实登录系统，这里返回一个固定的本地用户，让前端可以复用
...
def dashboard_options() -> dict[str, Any]:
    ...
    前端只展示这些选项，真正创建模型对象仍由 `agent.src.core.model` 负责。
    """
...
def _post_streaming_response(
    *, thread_id: str, repo_url: str, content: str
    ...
    # 先创建或更新 thread 业务记录。
    # 注意：这里不负责把用户消息写入 Store；
    # 用户消息展示由 SSE 的 user_message 事件负责，
    ...
    # 如果业务 Store 没有读到 thread，说明初始化失败；此时不能继续建立 SSE，否则前端会拿
    # 到残缺状态。
    ...
    async def event_iter():
        ...
        # 获取当前 FastAPI 请求所在的 asyncio 事件循环；worker 线程投递事件时需要回到这
        # 个 loop。
        loop = asyncio.get_running_loop()

        # 创建本轮 SSE 的内存队列。worker 线程负责生产事件，event_iter 负责消费并 yield
        # 给浏览器。
        ...
        def enqueue(event: str, data: dict[str, Any]) -> None:
            ...
            # worker 运行在普通线程，不能直接 await queue.put；必须通过事件循环线程安全
            # 投递。
        ...
        def event_sink(event: str, data: dict[str, Any]) -> None:
            ...
            if event == "text_delta":
                # 当前实现不再过滤英文/中文，只做统一字符串化，保留模型真实 assistant 输
                # 出。
                payload["content"] = _user_visible_stream_text(
        ...
        def worker() -> None:
            ...
            # worker 线程内部必须捕获异常，否则线程异常只会写到后端日志，前端收不到失败
            # 事件。
            try:
                # 真正运行 Agent 的入口。runtime 会做任务分类、方案确认、Agent 构建和事
                # 件流消费。
                run_agent_task(
                    # 本轮任务绑定的 Gitee 仓库地址，已经在外层 API 边界做过规范化。
                    repo_url=repo_url,
                    # 用户本轮真实输入，runtime 会根据任务类型决定
                    # 是否包装内部执行上下文。
                    prompt=content,
                ...
                # Agent 正常结束后，重新读取最新 thread 摘要，里面可能已经包含分支、PR、
                # 状态等新信息。
                latest = get_task(thread_id) or initial_task

                # 告诉前端本轮任务最终元信息；注意该 payload 不带 messages，不覆盖前端已
                # 有正文。
                enqueue("thread_done", _thread_meta_payload(latest))

                # 发送 done 事件，通知 event_iter 结束 while 循环，也通知前端结束
                # streaming 状态。
                enqueue("done", {})
            except Exception as exc:
                # 失败时也尽量读取最新 thread，保证前端能看到 failed 状态或错误后的元信
                # 息。
                latest = get_task(thread_id) or initial_task

                # 把异常转成 SSE error 事件。这里不做复杂格式化，
                # 详细堆栈仍然看后端日志。
        ...
        # 后续真正的模型 token 会通过 streaming_runtime.py
        # 继续创建或更新 assistant 消息。
        ...
        thread = threading.Thread(
            target=worker, name=f"FOREMAN-stream-{thread_id}", daemon=True
        )

        # 启动 worker 后，Agent 才真正开始运行；event_iter 继续留在 asyncio loop 里发送
        # SSE。
        thread.start()
...
async def dashboard_stream_new_message(
...
async def dashboard_stream_existing_message(
    ...
    repo_url = _normalize_dashboard_repo_url(
        body.repo, fallback=(task or {}).get("repo_url")
    )
    return _post_streaming_response(
        thread_id=thread_id, repo_url=repo_url, content=body.content
    )
