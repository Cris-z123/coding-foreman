...
def _task_kind_from_config() -> str:
    ...
    configurable = get_config().get("configurable", {})
    return (
        str(configurable.get("task_kind", "unknown"))
        ...
        if isinstance(content, list):
            ...
            for item in content:
                if isinstance(item, str):
                    ...
                    # LangChain/OpenAI 风格的分块内容通常把文本放在 text 或 content 字段
                    # 中。
...
class MemoryUpdateMiddleware(AgentMiddleware):
    ...
    def after_agent(self, state: AgentState, runtime: Any) -> dict[str, Any] | None:
        ...
        if not repo_url:
            logger.warning(
                "MemoryUpdateMiddleware: get_config 中没有 repo_url，跳过记忆更新"
            )
            return None
        ...
        if not final_text or any(
            marker in mask_token(final_text).lower() for marker in SENSITIVE_MARKERS
        ):
        ...
        update_repo_memory_from_text(
            ...
            update=RepoMemoryUpdate(
                task_kind=_task_kind_from_config(), final_text=final_text
            ),
        )
        return None
    ...
    async def aafter_agent(
        ...
        # DeepAgents 可能走同步或异步运行路径。异步钩子复用同步逻辑，保证两条路径行为一
        # 致。
