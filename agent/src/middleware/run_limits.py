...
它不依赖具体模型供应商，也不解析完整 token 计费信息，只基于 LangGraph 事件流做稳定的运行
保护。
"""
...
TASK_KIND_DEFAULT_LIMITS: dict[str, tuple[int, int]] = {
    ...
    "analysis": (120, 900),
    "planning": (120, 900),
    # review 需要读取规则、PR 上下文和 diff，通常比 analysis 略重，但不应进入长时间编码
    # 循环。
    "review": (180, 1200),
...
class AgentRunLimits:
    ...
    open-swe 的模型循环主要由 `ModelCallLimitMiddleware` 保护。FOREMAN
...
class AgentRunLimitTracker:
    ...
    def __init__(
        self, limits: AgentRunLimits | None = None, task_kind: str | None = None
    ):
    ...
    def observe_event(self, event: Any) -> None:
        ...
        if method in {"tool_calls", "tools"}:
            ...
            if method == "tool_calls" or event_name == "tool-started":
                # 兼容不同事件形态：有的流直接用 tool_calls，有的在 tools 事件里标记
                # tool-started。
                self.tool_calls += 1
