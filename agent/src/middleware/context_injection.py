    ...
    *,
    ...
    if memory_content:
        ...
        if len(trimmed) > MAX_REPO_MEMORY_CHARS:
            ...
            trimmed = (
                trimmed[:MAX_REPO_MEMORY_CHARS].rstrip() + "\n\n...（仓库记忆已截断）"
            )
        lines += [
            "",
...
class ContextInjectionMiddleware(AgentMiddleware):
    ...
    def before_agent(self, state: AgentState, runtime: Any) -> dict[str, Any] | None:
        ...
        if need_fallback_read:
            ...
            item = get_langgraph_store().get(
                namespace, repo_memory_store_key(repo.owner, repo.repo)
            )
            memory_content = (
                str(item.value.get("content") or "").strip()
                if item is not None
                else None
            )
        ...
        # 4. 组装面向模型的仓库上下文。这里会自动脱敏 token，防止日志或模型上下文泄露凭
        # 证。
