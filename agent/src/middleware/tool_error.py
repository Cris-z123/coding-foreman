...
def tool_error_result(
    ...
    *,
    ...
    if isinstance(error, WorkspacePermissionError):
        hint = (
            "只能访问工作区内路径。请使用 '/projects' 或 '/projects/仓库名' 这样的虚"
            "拟路径。"
        )
    elif isinstance(error, IsADirectoryError):
        hint = (
            "当前路径是目录。请先调用 ls（旧工具名 list_files）查看目录内容，再对具体"
            "文件调用 read_file 或 write_file。"
        )
    elif isinstance(error, FileNotFoundError):
        hint = "文件不存在。请先调用 ls 确认真实路径，再继续操作。"
    elif isinstance(error, PermissionError):
        hint = (
            "文件系统拒绝访问。请确认路径不是目录、没有被其他程序占用，并改用工作区内"
            "具体文件路径。"
        )
        ...
        hint = (
            "工具执行失败。请根据 error 字段调整参数，必要时先读取目录、文件或 Git 状"
            "态再重试。"
        )
        ...
        detail=json.dumps(
            {
                ...
                "args": {
                    name: mask_token(str(value)) for name, value in kwargs.items()
                },
                "error": error_detail,
            },
            ensure_ascii=False,
...
class ToolErrorMiddleware(AgentMiddleware):
    ...
    def _record_error(
        self, request: ToolCallRequest, tool_name: str, error: Exception
    ) -> None:
        """记录工具异常事件到运行事件表。
        ...
        _record_original_tool_error(
            thread_id, tool_name=tool_name, kwargs=kwargs, error=error
        )
        record_event(
            thread_id,
