...
LocalShellBackend 仍然是最终安全边界；
middleware 的职责是提前给模型返回更明确的中文反馈，
...
from agent.src.tools.gitee_api import mask_token, normalize_gitee_repo_url
...
def _reject_result(
    error: ToolInputRejected, *, tool_name: str, backend: LocalShellBackend
) -> dict[str, Any]:
    """把入参拦截结果转换成模型可读的中文结构。

    该结构会被序列化为 ToolMessage content。
    模型可以读取 `error` 和 `hint` 字段后修正参数，
    而前端也可以展示 `workspace` 辅助定位。
    """
    ...
    return {
        ...
        "hint": (
            "请改用工作区内相对路径，例如 '.'、'projects' 或 'projects/仓库名'；"
            "不要访问 .secrets。仓库地址请使用标准 Gitee HTTPS 地址。"
        ),
    }
...
def _get_thread_id(request: ToolCallRequest) -> str | None:
    ...
    middleware 不直接接收业务参数，
    而是从 LangGraph runtime config 中读取当前任务上下文。
...
def _is_windows_absolute_path(value: str) -> bool:
    ...
    if re.match(r"^[A-Za-z]:[\\/]", value):
        return True
    try:
        return PureWindowsPath(value).is_absolute() and bool(
...
def sanitize_workspace_path(
    value: Any, *, argument_name: str, backend: LocalShellBackend
) -> Any:
    """清洗路径类参数。
    ...
    if _is_windows_absolute_path(cleaned):
        ...
        raise ToolInputRejected(
            f"{argument_name} 不能使用工作区外的 Windows 绝对路径：{cleaned}"
        )
...
def sanitize_tool_kwargs(
    tool_name: str, kwargs: dict[str, Any], *, backend: LocalShellBackend
) -> dict[str, Any]:
    """根据参数名对工具入参做统一清洗。
    ...
    for key in PATH_ARGUMENTS:
        if key in sanitized:
            sanitized[key] = sanitize_workspace_path(
                sanitized[key], argument_name=key, backend=backend
            )
...
class SanitizeToolInputsMiddleware(AgentMiddleware):
    ...
    def __init__(self, *, backend: LocalShellBackend) -> None:
        super().__init__()
        # backend 提供 workspace 根目录和最终文件系统边界，路径清洗需要基于它判断绝对路
        # 径归属。
    ...
    def _sanitize_request(self, request: ToolCallRequest) -> ToolCallRequest:
        ...
        LangGraph 的 ToolCallRequest 是不可直接假定结构稳定的对象，所以先防御式读取
        tool_call。
    ...
    def _record_rejection(
        self, request: ToolCallRequest, tool_name: str, error: ToolInputRejected
    ) -> None:
        """把参数拒绝事件写入运行事件表。

        事件记录用于前端展示和问题排查；没有 thread_id 时跳过，不影响中间件返回
        ToolMessage。
        """
        ...
        record_event(
            ...
            status="error",
            detail=json.dumps(
                {"tool": tool_name, "error": mask_token(str(error))}, ensure_ascii=False
            ),
        )
    ...
    def wrap_tool_call(
        ...
        try:
            ...
            logger.warning(
                "工具入参被拒绝：tool=%s error=%s", tool_name, mask_token(str(exc))
            )
            self._record_rejection(request, tool_name, exc)
            return _reject_tool_message(
                exc, request=request, tool_name=tool_name, backend=self.backend
            )
