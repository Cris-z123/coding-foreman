...
def is_pull_only_task(prompt: str) -> bool:
    ...
    return _contains_any(normalized, pull_keywords) and not _contains_any(
        normalized, change_keywords
    )
...
def is_workspace_listing_task(prompt: str) -> bool:
    ...
    return _contains_any(normalized, list_keywords) and not _contains_any(
        normalized, change_keywords
    )
...
def _make_structured_intent_model():
    ...
    return make_intent_model().with_structured_output(
...
def _classify_by_keyword_backup(prompt: str) -> TaskKind:
    ...
    from agent.src.core.task_intent_keyword_backup import (
...
- review：用户要求代码审查、PR review、读取 PR diff、输出审查报告，
尤其是明确不要修改代码。
...
def _has_review_fix_plan_marker(normalized: str) -> bool:
    ...
    has_fix_plan = _contains_any(
        normalized, ["修复方案", "修改方案", "整改方案", "生成方案", "给我一个方案"]
    )
