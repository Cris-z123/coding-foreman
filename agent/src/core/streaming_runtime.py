...
raw protocol events 转换成前端能稳定消费的事件。
...
from agent.src.middleware.run_limits import AgentRunLimitExceeded, AgentRunLimitTracker
from agent.src.tools.gitee_api import mask_token
...
def _event_payloads(event: Any) -> list[Any]:
    ...
    if isinstance(data, tuple):
        ...
        # params.data = (payload, metadata)。第 0 项才是 content-block-delta
        # 等正文事件。
...
def _text_block_marker_from_event(event: Any) -> tuple[str, int | None] | None:
    ...
    if event_name == "content-block-start":
        ...
        # 有些版本的 text block start 不带 content.type；只要不是工具调用块，
        # 就按文本块处理。
    ...
    if event_name == "content-block-finish":
        ...
        content = payload.get("content")
        if isinstance(content, dict) and str(content.get("type") or "") in {
            "tool_call",
            "tool_call_chunk",
        }:
            return None
...
def _should_flush_stream_text(
    *, accumulated_text: str, last_flushed_length: int, delta: str
) -> bool:
    """判断是否需要把模型正文增量刷新到业务事件表。

    DeepAgents 的 raw message 事件可能按 token 级别返回，如果每个 token 都写一次
    SQLite，
...
def _extract_todos(tool_call: Any) -> list[dict[str, str]]:
    ...
    if isinstance(call_input, str):
        try:
            ...
            return (
                [{"content": call_input, "status": "pending"}]
                if call_input.strip()
                else []
            )
...
def _record_write_todos(
    thread_id: str, run_id: str, tool_call: Any, index: int
) -> bool:
    """只把 DeepAgents 内置 write_todos 转成结构化任务清单事件。"""

    tool_name = str(
        _safe_field(tool_call, "tool_name", "")
        or _safe_field(tool_call, "name", "")
        or ""
    )
    ...
    if not todos:
        return True
    call_id = str(
        _safe_field(tool_call, "id", "")
        or _safe_field(tool_call, "tool_call_id", "")
        or index
    )
    record_event(
        thread_id,
...
def _messages_from_output(output: Any) -> list[dict[str, Any]]:
    ...
    官方 Deep Agents 返回值通常是 `{"messages": [...]}`；为了稳定运行，
    这里也兼容对象属性和其它返回结构。
    """
...
def _consume_raw_event_stream(
    *,
    ...
    2. 同时继续读取 `method=tool_calls` 和 `method=subagents`，保留 write_todos
    任务计划、
       工具步骤和子 Agent 生命周期展示。

    如果某个 DeepAgents 小版本没有在 raw event 中暴露 tool_calls，工具内部的
    record_event
    仍然会记录读文件、命令、Gitee 等步骤；但 write_todos 只有 raw tool_calls
    可见时才会出现。
    """
    ...
    for event in stream:
        ...
        if marker is not None:
            ...
            if marker_kind == "start":
                ...
                if current_assistant_text and last_flushed_length != len(
                    current_assistant_text
                ):
        ...
        if delta:
            if current_assistant_index == 0:
                # 有些模型流不会显式给 content-block-start，这里按第一个 delta
                # 自动开块。
                assistant_message_index += 1
                current_assistant_index = assistant_message_index
                current_assistant_text = ""
        ...
        if tool_chunk is not None:
            ...
            if tool_name == "write_todos":
                ...
                call_id = str(
                    tool_chunk.get("id")
                    or tool_chunk.get("tool_call_id")
                    or "write_todos"
                )
                ...
                if isinstance(args, dict):
                    ...
                    payload_text = json.dumps(todos, ensure_ascii=False)
                    if todos and payload_text != write_todo_last_payload_by_call.get(
                        call_id
                    ):
                        _record_todos(
                            thread_id,
            ...
            if tool_name == "write_todos":
                ...
                if isinstance(tool_input, dict):
                    ...
                    if todos and payload_text != write_todo_last_payload_by_call.get(
                        ...
                        event_status = (
                            "completed"
                            if tool_event.get("event") == "tool-finished"
                            else "in_progress"
                        )
                        _record_todos(
                            thread_id,
...
def run_agent_with_event_stream(
    *,
    ...
    record_event(
        thread_id, "model", "调用 deepseek-v4-pro", kind="other", status="in_progress"
    )
    # raw protocol events 是当前版本里唯一能拿到 token/chunk 的通道。
    # 这里同时解析 text-delta 和 write_todos 的 tool_call_chunk，
    # 保证正文和任务计划都能流式更新。
    try:
    ...
    record_event(
        thread_id, "model", "调用 deepseek-v4-pro", kind="other", status="completed"
    )
    logger.info(
