...
from agent.src.utils.env_utils import get_env, require_env
...
def make_main_model() -> BaseChatModel:
    ...
    只保留一个主模型，默认对齐 open-swe 使用的 `deepseek-v4-pro`。
...
def make_intent_model() -> BaseChatModel:
    ...
    单独提供公共函数的好处是：模型初始化逻辑集中在 `agent.src.core.model`，业务模块
    ...
    return init_chat_model(
        ...
        base_url=require_env("DEEPSEEK_BASE_URL"),
        max_tokens=int(
            get_env("INTENT_MODEL_MAX_TOKENS", str(INTENT_MODEL_MAX_TOKENS))
        ),
        streaming=False,
        extra_body={"thinking": {"type": "disabled"}},
    )
