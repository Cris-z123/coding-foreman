...
# E:\my_project\FOREMAN，Linux/Docker 部署时通常是 /opt/FOREMAN/app。
# 后续所有本项目自己的数据文件、日志文件都默认放在这个目录下面，
# 避免 LangGraph dev 或第三方工具把文件散落到隐藏目录中。
PROJECT_ROOT = Path(__file__).resolve().parents[3]

# SQLite 数据目录：默认使用项目内 data/。
# 用户仍然可以通过 .env 覆盖，但推荐显式落在项目目录中，
# 方便讲课时直接打开 SQLite 文件观察 checkpoint 和业务 Store 的区别。
DATA_DIR = Path(get_env("FOREMAN_DATA_DIR", str(PROJECT_ROOT / "data"))).resolve()
...
LOG_DIR = Path(get_env("FOREMAN_LOG_DIR", str(PROJECT_ROOT / "logs"))).resolve()
LOG_LEVEL = get_env("FOREMAN_LOG_LEVEL", "INFO").upper()
LOG_ROTATION_WHEN = get_env("FOREMAN_LOG_WHEN", "midnight")
LOG_ROTATION_INTERVAL = int(get_env("FOREMAN_LOG_INTERVAL", "1"))
LOG_RETENTION_DAYS = int(get_env("FOREMAN_LOG_RETENTION_DAYS", "14"))
...
def _default_workspace_root() -> str:
    ...
    - Linux/Docker: /opt/FOREMAN/workspace
    - Windows: E:\ai_workspace

    默认值只用于本地开发兜底，避免代码里写死某一个操作系统路径。
    """

    if os.name == "nt":
        return r"E:\ai_workspace"
    return str(PROJECT_ROOT.parent / "ai_workspace")


# Agent 操作真实代码仓库时使用的工作区。它和项目源码目录分离，
# 避免 Agent 在执行用户任务时误改 FOREMAN 项目本身。
WORKSPACE_ROOT = (
    Path(get_env("AI_WORKSPACE_ROOT", _default_workspace_root())).expanduser().resolve()
)
PROJECTS_DIR = WORKSPACE_ROOT / "projects"

# DeepAgents skills 目录。统一放在 WORKSPACE_ROOT/skills，
