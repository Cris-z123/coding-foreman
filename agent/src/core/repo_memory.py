...
from agent.src.tools.gitee_api import GiteeRepo

REPO_MEMORY_NAMESPACE_PREFIX: Final[tuple[str, str]] = ("FOREMAN", "repo-memory")
...
def repo_project_dir(repo: GiteeRepo) -> str:
    ...
    第一版本不再维护“仓库 URL -> 本地目录”的 SQLite 映射表。
...
def _extract_owner_repo_from_namespace(
    namespace: tuple[str, ...],
) -> tuple[str, str] | None:
    """从 namespace 中提取 owner 和 repo。

    namespace 格式: ("FOREMAN", "repo-memory", owner, repo)
    """
...
def build_initial_repo_memory(*, repo: GiteeRepo, project_dir: str) -> str:
    ...
    command_repo_dir = (
        project_dir.replace("\\", "/").removeprefix("projects/").strip("/")
    )
...
- `execute` 默认在 `/projects` 对应的本地目录下执行，
  命令里直接使用 `{command_repo_dir}`，不要写 `projects/{command_repo_dir}`。
