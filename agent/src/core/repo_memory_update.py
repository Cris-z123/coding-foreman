...
from agent.src.tools.gitee_api import GiteeRepo, mask_token
...
def _extract_bullets(
    text: str, *, keywords: tuple[str, ...], limit: int = 8
) -> list[str]:
    """从最终回答中提取包含关键词的列表项。
...
def _extract_code_items(
    text: str, *, suffixes: tuple[str, ...], limit: int = 10
) -> list[str]:
    ...
    for item in re.findall(r"`([^`]+)`", text):
        ...
        if any(clean.endswith(suffix) for suffix in suffixes) or any(
            suffix in clean for suffix in suffixes
        ):
            if clean not in results:
...
def _extract_test_commands(text: str, *, limit: int = 5) -> list[str]:
    ...
    for item in re.findall(r"`([^`]+)`", text):
        ...
        ) and not _contains_sensitive_text(clean):
            if clean not in results:
                results.append(mask_token(clean))
        if len(results) >= limit:
            return results
    ...
    for raw_line in text.splitlines():
        ...
        if lowered.startswith(
            ("python -m pytest", "pytest")
        ) and not _contains_sensitive_text(line):
            clean = mask_token(line)
            if clean not in results:
                results.append(clean)
        if len(results) >= limit:
...
def build_updated_repo_memory(memory: str, update: RepoMemoryUpdate) -> str:
    ...
    key_files = _extract_code_items(
        text, suffixes=(".py", ".md", ".txt", ".html", ".json", ".toml"), limit=10
    )
    ...
    completed_features = [
        ...
        if not any(
            skip in item
            for skip in ("分支", "Pull Request", " PR", "Fast-forward", "merge")
        )
    ]
...
def update_repo_memory_from_text(
    *,
    ...
    if updated == current:
        logger.info(
            "仓库记忆无新增稳定结论：repo=%s/%s task_kind=%s",
            repo.owner,
            repo.repo,
            update.task_kind,
        )
    ...
    logger.info(
        "仓库记忆已结构化更新：repo=%s/%s task_kind=%s",
        repo.owner,
        repo.repo,
        update.task_kind,
    )
