...
这个文件是 FastAPI 版本 FOREMAN 的任务调度中心，主要负责把一次用户输入
...
from agent.src.core.checkpoint_history import visible_checkpoint_messages
from agent.src.core.events import record_event
from agent.src.core.graph import get_checkpointer, get_langgraph_store, get_store
from agent.src.core.repo_memory import ensure_repo_memory_initialized, repo_project_dir
from agent.src.core.repo_memory_update import (
    RepoMemoryUpdate,
    update_repo_memory_from_text,
)
from agent.src.core.settings import PROJECTS_DIR, WORKSPACE_ROOT
from agent.src.core.streaming_runtime import run_agent_with_event_stream
from agent.src.core.task_intent import (
    classify_task_kind,
    is_pull_only_task,
    is_workspace_listing_task,
)
from agent.src.server import get_agent
from agent.src.tools.gitee_api import mask_token, parse_gitee_repo_url
...
def _build_agent_for_runtime(
    *,
    ...
    open-swe 由 LangGraph Server 注入 `RunnableConfig`。不用
...
def _build_agent_user_content(
    *,
    ...
    if task_kind == "coding":
        ...
        if approved_plan:
            plan_instruction = (
                "\n\n用户已经确认以下技术方案，请按该方案实施；"
                "如执行中发现必要调整，请在最终总结中说明：\n"
                f"{approved_plan}"
            )
        task_instruction = (
            "这是开发实现任务。请按系统开发流程完成任务，必要时修改代码、验证，"
            "并创建或复用 Gitee Pull Request。"
            f"{plan_instruction}"
        )
    else:
        task_instruction = (
    ...
    *,
    ...
    if previous_plan and revision_prompt:
        return (
            ...
            "请只生成技术方案，不要修改文件、不要提交、不要 push、不要创建 Pull "
            "Request。\n"
...
def _revision_source_prompt(
    *, previous_source_prompt: str, revision_prompt: str
) -> str:
...
def initialize_task_record(
    *,
    ...
    # 如果是全新会话，由 runtime 生成 thread_id；如果是继续对话，复用前端传入的
    # thread_id。
...
def run_workspace_listing_task(
    *, repo_url: str, prompt: str, thread_id: str | None = None
) -> dict[str, Any]:
    """直接列出本地工作区项目，不调用模型。
    ...
    try:
        # 所有可见步骤都通过 record_event 写入 run_events，
        # 前端可以复用同一套事件展示组件。
        record_event(
            thread_id,
            "workspace",
        ...
        store.record_run(
            run_id=run_id, thread_id=thread_id, status="completed", finished=True
        )
        record_event(thread_id, "done", "任务完成", status="completed")
        logger.info(
            "工作区项目查询完成：thread_id=%s projects=%s", thread_id, len(projects)
        )
        return {
            "thread_id": thread_id,
            "run_id": run_id,
        ...
        record_event(
            thread_id, "failed", "任务失败", status="error", detail=mask_token(str(exc))
        )
        store.record_run(
            run_id=run_id,
            thread_id=thread_id,
...
def run_pull_only_task(
    *, repo_url: str, prompt: str, thread_id: str | None = None
    ...
    try:
        logger.info(
            "开始执行 pull-only 任务：thread_id=%s repo=%s/%s",
            thread_id,
            repo.owner,
            repo.repo,
        )
        ...
        if target.exists() and (target / ".git").exists():
            ...
            record_event(
                thread_id, "sync", "同步远程仓库", kind="execute", status="in_progress"
            )
            remote_result = backend.run(
                f"git remote set-url origin {clone_url}",
                cwd=str(relative_dir),
                timeout=60,
            )
            checkout_result = backend.run(
                "git checkout master", cwd=str(relative_dir), timeout=300
            )
            ...
            # 目录固定为 projects/<repo>，不再写入仓库目录映射表。
            record_event(thread_id, "workspace", "准备本地工作区", status="completed")
            record_event(
                thread_id,
                "sync",
            ...
            clone_result = backend.run(
                f"git clone {clone_url} {target.name}", cwd="projects", timeout=600
            )
        ...
        if failed is not None:
            ...
            record_event(
                thread_id, "sync", "同步远程仓库", kind="execute", status="error"
            )
            raise RuntimeError(
                f"git pull 失败: {mask_token(failed.stderr or failed.stdout)}"
            )

        record_event(
            thread_id, "sync", "同步远程仓库", kind="execute", status="completed"
        )
        store.update_thread_status(thread_id, "completed")
        store.record_run(
            run_id=run_id, thread_id=thread_id, status="completed", finished=True
        ...
        record_event(thread_id, "done", "任务完成", status="completed")
        logger.info(
            "pull-only 任务完成：thread_id=%s repo_dir=%s", thread_id, relative_dir
        )
...
def run_plan_response_task(
    *,
    ...
    agent = _build_agent_for_runtime(
        thread_id=thread_id, task_kind="planning", repo_url=repo.clone_url
    )
    ...
    *,
    ...
    if is_workspace_listing_task(prompt):
        return run_workspace_listing_task(
            repo_url=repo_url, prompt=prompt, thread_id=thread_id
        )
    ...
    # display_prompt 是本轮用户真实输入，用于前端展示；coding_prompt 是传给 Agent
    # 的执行目标。
    # 在“确认实施”场景下，二者不能混用：用户真实输入可能只有“确认”，
    # 而执行目标应是上一轮方案。
    ...
    if existing_thread and _is_approval_prompt(prompt):
        ...
        # 再用该方案的 source_prompt 还原用户最初的开发需求，
        # 避免把“确认”当作新需求执行。
        ...
        if plan_message is not None:
            ...
            coding_prompt = str(
                ...
                or _latest_non_approval_user_prompt(
                    thread_id, existing_thread.get("user_prompt") or prompt
                )
            )
            task_kind = "coding"
        ...
        plan_message = (
            _latest_confirmable_plan_message(thread_id)
            if _is_plan_revision_prompt(prompt)
            else None
        )
        if plan_message is not None and prompt.strip():
            ...
            return run_plan_response_task(
                repo_url=repo_url,
                prompt=_plan_source_prompt(
                    plan_message, _latest_non_approval_user_prompt(thread_id, prompt)
                ),
                thread_id=thread_id,
    ...
    if task_kind == "coding" and approved_plan_text is None:
        ...
        return run_plan_response_task(
            repo_url=repo_url, prompt=prompt, thread_id=thread_id, event_sink=event_sink
        )
    ...
    agent = _build_agent_for_runtime(
        thread_id=thread_id, task_kind=task_kind, repo_url=repo.clone_url
    )
    ...
    try:
        ...
        if final_answer:
            ...
            # branch/pr 信息来自 Store 中的最新业务状态，通常由 Gitee
            # 工具在执行过程中写入。
        ...
        logger.info(
            "任务完成：thread_id=%s run_id=%s messages=%s",
            thread_id,
            run_id,
            len(messages),
        )
