...
def is_workspace_listing_task(prompt: str) -> bool:
    ...
    return any(
        ...
        for keyword in ["有哪些项目", "工作目录", "本地工作", "workspace"]
    ) and not any(
        keyword in normalized
        for keyword in ["修改", "修复", "创建", "提交", "push", "pr"]
    )
...
def classify_task_kind(prompt: str) -> TaskKind:
    ...
    has_negative_change_marker = any(
        marker in normalized for marker in negative_change_markers
    )
    if (
        has_review_fix_context
        and any(marker in normalized for marker in approval_markers)
        and not has_negative_change_marker
    ):
        return "coding"
    ...
    is_explicit_review_only = has_review and any(
        marker in normalized for marker in review_only_markers
    )
    ...
    has_review_coding_override = any(
        keyword in normalized for keyword in review_coding_override_keywords
    )
    ...
    if has_review and (is_explicit_review_only or has_negative_change_marker):
        return "review"
    if has_review and (
        not has_coding or is_explicit_review_only or not has_review_coding_override
    ):
        return "review"
