...
from agent.src.core.persistence import make_checkpointer, make_langgraph_store
...
from agent.src.store import LocalSqliteStore
...
def get_checkpointer():
    ...
    显式把 checkpoint 写到 data/checkpoints.sqlite，方便课堂演示重启恢复。
    """
...
def build_agent(thread_id: str, task_kind: TaskKind = "coding"):
    ...
    open-swe 的 LangGraph Server 入口。保留这个函数是为了兼容脚本和已有
    FastAPI runtime 调用点。
    """
