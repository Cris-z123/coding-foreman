...
网络访问统一通过 `safe_http.request_with_safe_redirects()`，
在请求前和每次重定向前校验目标地址，
避免工具访问内网、本机地址或保留地址。
"""
...
from agent.src.tools.runtime_context import get_runtime_thread_id
from agent.src.tools.safe_http import request_with_safe_redirects
...
def fetch_url(url: str, timeout: int = 30) -> dict[str, Any]:
    ...
    try:
        ...
        result = {
            "ok": False,
            "url": normalized_url,
            "error": f"网页读取失败：{mask_token(str(exc))}",
        }
    except Exception as exc:  # noqa: BLE001 - 工具层返回可恢复错误，由 middleware 再做兜底
        # 其他异常写 warning 日志，并对错误文本做 token 脱敏。
        logger.warning(
            "读取网页资料失败：url=%s error=%s", normalized_url, mask_token(str(exc))
        )
        result = {
            "ok": False,
            "url": normalized_url,
            "error": f"网页处理失败：{mask_token(str(exc))}",
        }
