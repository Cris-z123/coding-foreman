...
from agent.src.tools.gitee_api import (
    create_pull_request,
    get_pull_request,
...
from agent.src.tools.runtime_context import (
    get_runtime_thread_id,
    runtime_is_read_only_task,
)
...
def open_gitee_pull_request(
    ...
    head: str,
    base: str = "master",
    title: str = "FOREMAN generated changes",
    body: str = "由 FOREMAN 自动生成。",
) -> dict[str, Any]:
    ...
    Returns:
        成功时返回 `ok=True`、PR URL 和原始 API 数据；只读任务或 API
        失败时返回错误信息。
    """
    ...
    if runtime_is_read_only_task():
        ...
        return {
            "ok": False,
            "error": (
                "当前任务是只读任务，不能创建 Pull Request。"
                "请先向用户输出方案或分析结论，等待确认实施。"
            ),
        }
    ...
    if thread_id:
        ...
        get_store().update_thread_status(
            thread_id, "pr_created", pr_url=pr_url, branch_name=head
        )
        record_event(
            thread_id,
            "gitee:pr",
    ...
    if pr.get("reused"):
        ...
        logger.info(
            "Gitee PR 已存在，复用已有 PR：thread_id=%s pr_url=%s", thread_id, pr_url
        )
    else:
...
def publish_gitee_pr_comment(
    ...
    logger.info(
        "准备发布 Gitee PR 评论：owner=%s repo=%s number=%s", owner, repo, number
    )
    return post_pr_comment(owner=owner, repo=repo, number=number, body=body)


@tool
def get_gitee_pull_request_context(
