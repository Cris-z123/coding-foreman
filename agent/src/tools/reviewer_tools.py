...
from agent.src.utils.reviewer_diff import (
...
DEFAULT_RULES_PATH = (
    Path(__file__).resolve().parents[1] / "reviewer_rules" / "default_review_rules.md"
)
...
def load_default_review_rules() -> dict[str, Any]:
    ...
    source = "agent/src/reviewer_rules/default_review_rules.md"
    return {
...
@tool
def get_review_diff_summary(
    repo_dir: str, base: str = "master", head: str | None = None
) -> dict[str, Any]:
...
def validate_review_finding_location(
    raw_diff: str, file: str, line: int | None = None
) -> dict[str, Any]:
