...
def parse_gitee_repo_url(repo_url: str) -> GiteeRepo:
    ...
    Args:
        repo_url: 用户输入或任务配置中的 Gitee 仓库地址，支持带 `.git` 后缀的 HTTPS
        地址。
    ...
    return GiteeRepo(
        owner=owner, repo=repo, clone_url=f"https://gitee.com/{owner}/{repo}.git"
    )
...
def get_gitee_token() -> str:
    ...
    if not token:
        raise RuntimeError(
            "Missing required environment variable: GITEE_TOKEN or SCM_GITEE_TOKEN"
        )
    return token
    ...
    *,
    ...
    if response.status_code >= 400:
        ...
        if existing is not None:
            return existing
        raise RuntimeError(
            f"Gitee 创建 PR 失败: {response.status_code} {response.text}"
        )
...
def _gitee_get(path: str, *, params: dict | None = None) -> dict | list:
    ...
    if response.status_code >= 400:
        raise RuntimeError(
            f"Gitee API 读取失败: {response.status_code} {response.text}"
        )
    return response.json()
...
def post_pr_comment(*, owner: str, repo: str, number: int, body: str) -> dict:
    ...
    if response.status_code >= 400:
        raise RuntimeError(
            f"Gitee 发布 PR 评论失败: {response.status_code} {response.text}"
        )
    return response.json()
