...
def main() -> None:
    ...
    if "repo-bootstrap-analysis" not in prompt or "fetch_url" not in prompt:
        raise AssertionError(
            "系统提示词没有包含 repo-bootstrap-analysis 或 fetch_url 使用规则"
        )
    ...
    coding_missing = [
        term for term in coding_required_terms if term not in coding_content
    ]
    if coding_missing:
        raise AssertionError(
            f"ai-coding-implementation skill 缺少关键内容：{coding_missing}"
        )
