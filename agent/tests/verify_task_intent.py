...
from agent.src.core.task_intent import classify_task_kind
from agent.src.tools import (
    web_search,
)
...
def main() -> None:
    ...
    if qa_task_kind != "qa":
        raise AssertionError(
            f"普通问答不应进入方案或编码流程：{qa_prompt} -> {qa_task_kind}"
        )
    ...
    for coding_prompt in coding_prompts:
        ...
        if detected != "coding":
            raise AssertionError(
                f"改造类任务应识别为 coding：{coding_prompt} -> {detected}"
            )
    ...
    if leaked:
        raise AssertionError(
            f"工具列表暴露了已移除的仓库/Git 操作工具：{sorted(leaked)}"
        )
