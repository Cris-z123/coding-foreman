...
def inspect_raw(agent: Any, *, prompt: str, thread_id: str, limit: int) -> None:
    ...
    for index, event in enumerate(stream):
        ...
        print(
            f"[raw {index}] "
            f"{json.dumps(_summarize_raw_event(event), ensure_ascii=False)}"
        )
    try:
...
def inspect_interleave(agent: Any, *, prompt: str, thread_id: str, limit: int) -> None:
    ...
    for index, (name, item) in enumerate(
        stream.interleave("messages", "tool_calls", "subagents")
    ):
        if index >= limit:
...
def inspect_projection(
    agent: Any, *, prompt: str, thread_id: str, projection: str, limit: int
) -> None:
...
def main() -> None:
    ...
    parser.add_argument(
        "--task-kind",
        default="planning",
        choices=["planning", "analysis", "qa", "coding"],
    )
    parser.add_argument("--limit", type=int, default=80)
    parser.add_argument(
        "--mode",
        choices=["raw", "interleave", "messages", "tool_calls", "all"],
        default="all",
    )
    ...
    if args.mode in {"interleave", "all"}:
        inspect_interleave(
            agent, prompt=args.prompt, thread_id=thread_id, limit=args.limit
        )
